...
## Smoke-10 Generation Variance (2026-06-29)

A verification re-run `20260629-131400-kubernetes-smoke` with the same config and same restored index (no code, prompt, or retrieval change) produced `incomplete_answer` 3 instead of `weak_citations` 1. Retrieval metrics were identical across both runs (Doc Recall@1 0.700, @3 0.900, @5 0.900, Span Recall@5 1.000, MRR@5 0.800, Hit@5 1.000, Graded MRR@5 0.850), confirming retrieval is deterministic and reproducible over the index.

The three `incomplete_answer` failures (`deployments`, `troubleshooting-failedscheduling`, `troubleshooting-deployment-rollout`) each dropped required-points coverage from 1.0 to 0.667. In every case the expected evidence was retrieved but the generated answer omitted one required point: the Deployment answer described Pods lifecycle and scaling without mentioning ReplicaSets (evidence `concepts/workloads/controllers/deployment` was at rank 2); the other two similarly omitted a third required point despite citing the relevant docs. The required-points grader is deterministic (token-overlap aliases), so the variance is entirely in the LLM generation pass (`openai/gpt-oss-120b:nitro` is non-deterministic across calls). The PVC example flipped the other way, from `weak_citations` to passing, because citation coverage depends on which docs the generation chooses to cite.

Interpretation: retrieval strength is stable and reproducible; generation completeness is the variance source. The `incomplete_answer` bucket is fragile against generation omissions even when evidence is retrieved, and lexical metrics (ROUGE-L 0.161 vs 0.149, F1 0.204 vs 0.203) move only slightly while failure labels move more. This is why the project avoids tuning only for ROUGE/F1 and tracks required-points coverage and failure labels instead.

Named follow-up: stabilize answer completeness with a stricter generation prompt or a lightweight post-generation required-points checklist that forces cited docs' concrete checklist items into the final answer. This is a generation-side fix; no reindex or retrieval change is warranted. `20260629-002315-kubernetes-smoke` remains the canonical headline baseline; single-run `incomplete_answer` counts over unchanged retrieval are treated as generation variance, not a retrieval regression.

Demo trace candidates from the canonical headline run `20260629-002315-kubernetes-smoke`: success is `kubernetes::pods::turn_2` (decision `answer`, cited `concepts/workloads/pods`, no failure); failure is `kubernetes::troubleshooting-pvc-pending::turn_2` (`weak_citations`, well-grounded answer citing acceptable storage docs, legacy citation coverage expects the gold `dynamic-provisioning` doc).
